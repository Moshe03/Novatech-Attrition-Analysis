...
The raw dataset was cleaned and feature-engineered in Python before modelling in Power BI. Key decisions, all logged in the [cleaning notebook](NovaTech_Analysis_Workforce_Attrition.ipynb):
...
- **Cost estimation, an explicit assumption:** the dataset contains no salary data, so replacement cost is a **proxy**, using UK sector midpoints by job level (CIPD benchmarks) multiplied by the industry-standard 60% replacement rate. The £7.79M figure is therefore an informed estimate, not actuals, and should be read as directional.
...
![NovaTech attrition dashboard, Overview page](page1-overview.png)
...
![Attrition cost and financial impact page](page2-cost.png)

**3. Early-tenure and junior staff leave fastest.**
Attrition is steepest in the first six months (**29.4%**), easing to 20.6% at 7 to 12 months. The earliest-tenure staff are the highest exit risk, and analyst-level attrition was the highest of any job level. This reframes the problem from "retain everyone" to "fix the first six months, especially in customer-facing roles."
